...
14. **D10 Muon hybrid optimizer shipped 2026-05-18** ([commit `6e58cc4`](#)) — code only; GPU smoke pending. Used **upstream `optax.contrib.muon`** (shipped 0.2.5+) per verified review follow-up; floor bumped. Hidden bucket of existing 3-bucket muP `multi_transform` routes to Muon; embed/head/norms stay on AdamW. MuonClip standalone at `src/myllm/training/muonclip.py`. 9 new tests.
15. **D10 Muon validated on 2×B200 2026-05-20** — 4-mode smoke on B200 (adamw_baseline / muon_singlegpu / muon_fsdp / muon_no_mup_scale at 500 steps). All clean, no NaN, FSDP composes. **Key A/B test**: Muon with the muP `1/width_mult` scaling ON descends at only 63% of AdamW's rate over 50 steps (Δ +2.29 vs +3.67). Turning the scaling OFF (`muon_disable_mup_scale=True`) closes the gap to ~92% at 50 steps, then **fully matches AdamW by 500 steps** (Muon Δ +5.6826 vs AdamW Δ +5.6735 — nominally 0.2% ahead). The post-review DISPUTED claim that Muon's spectral-norm bound makes muP redundant for hidden matrices is **empirically validated** on our stack. **Stage 2 ships with `optimizer.type: muon_hybrid` + `muon_disable_mup_scale: true` locked in `configs/base_1b.yaml`**. First public JAX-Muon at >=1B is now de-risked.
...
| **D10** | Muon optimizer port (hybrid: Muon for hidden weights via optax.contrib.muon, AdamW for embeddings/head/scalars) + MuonClip | **Code DONE 2026-05-18** — GPU smoke pending ~$10 | Verified review (2026-05-18 follow-up agent) showed: use **upstream `optax.contrib.muon`** (shipped 0.2.5+); pinned optax floor; reuses existing 3-bucket muP `multi_transform` (swap hidden → Muon); MuonClip vendored at `src/myllm/training/muonclip.py` (post-step W_q/W_k rescale, threshold 100, alpha 0.5). Tests: param routing unchanged + single-step finite + Orbax-serializable state + MuonClip 4-case suite. **NO public JAX-Muon training at >=1B exists as of 2026-05-18 — Stage 2 with Muon = first run; treat as smoke.** Default still `type: adamw` in base_1b.yaml; flip to `muon_hybrid` or pass `--use-muon` to opt in. |
