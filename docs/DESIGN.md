...
- **D8**: ~~Investigation pending~~ — **MITIGATED 2026-05-18**. CPU audit + fp32-logsumexp mitigation (commit `d56e1e0`) + single-GPU verification on 1×B200 (all 3 modes clean at mb=1/seq=4K). Production-shape verification (4×B200 + mb=4 + seq=8K + FSDP) deferred to Stage 3 prep ($25-30). Stage 2 stays on full-CE — chunked-CE only matters at 7B+ where full-CE OOMs. Full writeup: [design/d8_chunked_ce_audit.md](design/d8_chunked_ce_audit.md).
...
| **D8** | ~~chunked-CE NaN-grad at 1B+B200+bf16~~ — **MITIGATED 2026-05-18**: CPU audit + fp32-logsumexp + single-GPU verification | Single-GPU done (~$3); production-shape ($25-30) deferred to Stage 3 prep | All 3 modes (baseline / disable-remat / fp32-cce) clean on 1×B200 at mb=1/seq=4K through 10 steps each. fp32 logsumexp produces finite grads. Stage 2 stays on full-CE. See [`design/d8_chunked_ce_audit.md`](design/d8_chunked_ce_audit.md) GPU verification §. |
...
| **D10** | Muon optimizer hybrid port (Muon hidden via `optax.contrib.muon`; AdamW embeddings/head/scalars) + MuonClip safety net | **Code DONE 2026-05-18**; ~$10 1×B200 smoke pending | Implemented via upstream `optax.contrib.muon` (shipped 0.2.5+; floor bumped). Reuses existing 3-bucket muP `multi_transform`. MuonClip standalone at `src/myllm/training/muonclip.py`. Default: `type: adamw` (opt in via `type: muon_hybrid` or `--use-muon`). No public JAX-Muon training at >=1B exists yet — Stage 2 with Muon = first run. |
